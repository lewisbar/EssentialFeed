...
final class CommentsUIComposer {
    ...
    static func commentsComposedWith(commentsLoader: @escaping () -> AnyPublisher<[ImageComment], Error>) -> ListViewController {
...
final class CommentsViewAdapter: ResourceView {
    ...
    func display(_ viewModel: ImageCommentsViewModel) {
        controller?.display(viewModel.comments.map { commentViewModel in
            CellController(id: commentViewModel, ImageCommentCellController(model: commentViewModel))
        })
    }
}
