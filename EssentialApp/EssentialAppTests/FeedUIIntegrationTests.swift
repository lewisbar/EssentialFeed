...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once load more action completes successfully")

        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator on second load more action")

        loader.completeLoadMoreWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once load more action completes with error")
    }
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no state change for the second view's loading indicator when first image loading completes successfully")

        loader.completeImageLoading(at: 1)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for first view when second image loading completes with error")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for second view when second image loading completes with error")
    }
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view when first image laoding completes successfully")
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, true, "Expected retry action for second view once second image laoding completes with error")
    }
    ...
    func test_feedImageViewRetryAction_retriesImageLoad() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url, image0.url, image1.url], "Expected fourth imageURL after second view retry action")
    }
